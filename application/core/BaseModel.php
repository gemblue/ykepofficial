...
class BaseModel extends \CI_Model
{
    ...
    */
    ...
    public function _get_rules($action=NULL)
    {
        ...
        // For handling $this->insert_rules
        if($this->is_assoc($this->insert_rules))
            $this->rules['insert'] = $this->rules['update'] = $this->insert_rules;
    ...
    {
        return $this->validate($rules, $additional_values, $row_fields_to_update);
    }
     ...
     * @return $this
     */
    public function validate($rules = NULL,$additional_values = NULL, $row_fields_to_update = array())
    {
        ...
        if($this->form_validation->run())
        {
            ...
            foreach($rules as $rule)
            {
                ...
                {
                    $this->validated[$rule['field']] = isset($this->formdata[$rule['field']]) 
                                                       ? $this->formdata[$rule['field']]
                                                       : $this->input->post($rule['field'], true);
                }
            }
            ...
            if(!empty($row_fields_to_update))
            {
                foreach ($row_fields_to_update as $key => $field) {
                    if (in_array($field, $this->fillable)) {
                        $this->row_fields_to_update[$field] = $this->input->post($field, true);
                    }
                ...
                if(isset($this->{$option}) && !empty($this->{$option}))
                {
                    foreach($this->{$option} as $key => $relation)
                    {
                        ...
                        if(!is_array($relation))
                        {
                            ...
                            $model = $this->_parse_model_namespace($foreign_model);
    ...
    public function paginate($rows_per_page, $total_rows = NULL, $page_number = 1, $customUri = false)
    {
        ...
        else
        {
            ...
            $this->previous_page = $this->pagination_delimiters[0].anchor($uri_string.$query_string, $this->pagination_arrows[0], $this->pagination_attr).$this->pagination_delimiters[1];
        }
        $uri_array[$segments] = $next_page;
        $uri_string = implode('/',$uri_array);
        if(isset($total_rows) && (ceil($total_rows/$rows_per_page) == $page_number))
        {
            // Just empty next page link
            $this->next_page = "";
            // $this->next_page = $this->pagination_delimiters[0].$this->pagination_arrows[1].$this->pagination_delimiters[1];
        }
        else
        {
            $this->next_page = $this->pagination_delimiters[0].anchor($uri_string.$query_string, $this->pagination_arrows[1], $this->pagination_attr).$this->pagination_delimiters[1];
        }
        ...
        if(isset($total_rows))
        {
            ...
            if($number_of_pages > 1)
            {
                // Set start page
                $startpage = 1;
                if($number_of_pages > 3 && $page_number > 3)
                    $startpage = $page_number - 3;

                // Set end page
                $endpage = $page_number + 3 <= $number_of_pages 
                            ? $page_number + 3 : $number_of_pages;
                if($number_of_pages <= 6) // Maximum page number appeared
                    $endpage = $number_of_pages;

                // Begin create pagination link
                ...
                for ($i = $startpage; $i <= $endpage; $i++) {
                    ...
                    $links .= (($page_number == $i) 
                              ? '<strong '.$this->pagination_attr.'>'.$i.'</strong>' 
                              : anchor($uri_string . '/' . $i . $query_string, $i, $this->pagination_attr));
            ...
            {
                // Just empty if page is only one
                $this->all_pages = '';
    ...
    /**
     * private function _parse_model_namespace($foreign_model)
     *
    ...
    private function _parse_model_namespace($foreign_model)
    {
        ...
        $data['foreign_model_name'] = array_pop($full_model);
