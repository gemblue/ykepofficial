 ...
 * My Model. 
 * 
 * Query Builder with added feature.
 * 
 * @license MIT
...
class MY_Model extends CI_Model
{
    ...
    public $fields = [];
    public $show_on_table = [];
    protected $rules = []; // Insert rule (and update, if $update_rules not set)
     ...
     *  'type' => 'has_one|has_many|has_many_pivot',
     *  'format' => [modelname, foreign_key, local_key]  
     * ]
    */
    public function __construct($table = null, $fields = [], $relationships = [])
    {
        $this->table = !empty($table) ? $table : $this->table;
        $this->fields = !empty($fields) ? $fields : $this->fields;
        if(!empty($relationships))
            foreach ($relationships as $relName => $opt)
                $this->{$opt['type']}[$relName] = $opt['format'];
    ...
    {
        // Default handling
        if($this->is_assoc($this->fields))
            $this->rules['insert'] = $this->rules['update'] = $this->fields;

        // For handling $this->update_rules
        if($this->is_assoc($this->update_rules))
            $this->rules['update'] = $this->update_rules;
     ...
     * if it is not set, validation data will use default $_POST
     * originated from form_validation library
     *
     * @param array $data
     */
    public function set_form_data($data = [])
    {
        if(is_array($data) && !empty($data)){
            // TODO: check non-existed data with default value from field structure
    ...
    // This method only used for backward compatibility. Use validate() instead
     ...
     * @param null $rules Gets the validation rules. If nothing is passed (NULL), will look for the validation rules inside the model $rules public property
     * @return $this
     */
    public function validate($rules = NULL)
    {
        ...
        if(empty($this->formdata))
            $this->set_form_data($this->input->post(null, true));
        
        // If $rules contain string 'insert' or 'update'
        if(is_string($rules) && in_array($rules, ['insert','update']))
            $rules = $this->_get_rules($rules);
        
        // If $rules not set, use insert rules
        else if(!isset($rules))
            $rules = $this->_get_rules('insert');

        foreach ($rules as $rule_field => $rule_value)
            if(!isset($rule_value['rules']))
                unset($rules[$rule_field]);

        if(empty($rules))
            throw new Whoops\Exception\ErrorException("Rules is not defined");

        // Set formdata to flash and set for validation
        $this->session->set_flashdata($this->formdata);
        ...
        if($this->form_validation->run() === true)
        {
            ...
            foreach($this->formdata as $field_data => $value_data)
                if(in_array($field_data, $this->fillable))
                    $this->validated[$field_data] = $value_data;
    ...
    public function insert($data = NULL)
    {
        ...
        if($multi === FALSE)
        {
            ...
            if($this->_database->insert($this->table, $data))
            {
                ...
                $return = $this->trigger('after_create',array_merge($data, ['id' => $id]));
                return $return;
            }
            return FALSE;
        }
    ...
    public function update_batch($data, $where_field)
    {
        $this->_database->update_batch($this->table, $data, $where_field);
        return $this->_database->affected_rows();
    }

    public function save_pivot($local_id, array $pivot_configs, array $foreign_ids = [])
    {
        // delete existing pivot foreign id if exist
        $this->db->where($pivot_configs['pivot_local_key'], $local_id)->delete($pivot_configs['pivot_table']);

        // rewrite new pivot foreign ids
        ...
        foreach ($foreign_ids as $fid) {
            $data[] = [
                $pivot_configs['pivot_local_key'] => $local_id,
                $pivot_configs['pivot_foreign_key'] => $fid
            ];
        }
        $this->db->insert_batch($pivot_configs['pivot_table'], $data);
        return $this->db->affected_rows();
    }

    public function update_or_insert($data, $column_name_where)
    {
        if (is_array($column_name_where))
        {
            $this->where($column_name_where);
        ...
        if($query->num_rows() == 1)
        {
            return $this->update($data, $column_name_where);
        } 
    ...
    public function find_or_insert($data, $column_name_where)
    {
        if (is_array($column_name_where))
        {
            $this->where($column_name_where);
    ...
    {
        $this->_database->where_in($field, $array);
        return $this;
    }

    public function where_not_in($field, $array)
    {
        $this->_database->where_not_in($field, $array);
        return $this;
    }
    ...
    public function get($where = NULL)
    {
        ...
        else
        {
            ...
            if ($query->num_rows() == 1)
            {
                $row = $query->row(0, $this->return_as);
    ...
    public function get_all($where = NULL){return $this->getAll($where);}
    ...
    public function count_rows($where = NULL)
    {
        ...
        else
        {
            ...
            $this->_reset_trashed();
            $this->_write_to_cache($number_rows);
            return $number_rows;
        }
    }
            ...
            if(isset($sub_results) && !empty($sub_results)) {
                ...
                foreach ($sub_results as $result) {
                    ...
                    if(isset($pivot_table))
                    {
                        ...
                        if(isset($get_relate) and $get_relate !== FALSE)
                        {
                            if(!is_bool($get_relate))
                                $this->{$relation['foreign_model_name']}->fields($get_relate);

                            $subs[$the_local_key][$the_foreign_key] = $this->{$relation['foreign_model_name']}->where($foreign_key, $result[$foreign_key])->get();
                        }
                        else
                        {
                        ...
                        else
                        {
                            if($this->is_assoc($relation))
                            {
                                ...
                                if($option=='has_many_pivot')
                                {
                                    ...
                                    $get_relate = $relation['get_relate'] ?? FALSE;
                                }
    ...
    // Backward compatibility
    public function fields($fields = NULL){return $this->select($fields);}
    ...
    protected function _write_to_cache($data, $cache_name = NULL)
    {
        if(isset($cache_name) || (isset($this->_cache) && !empty($this->_cache)))
        {
            ...
            {
                cache()->set($cache_name, $data, $seconds);
     ...
     * HELPER FUNCTIONS
     */
    public function paginate($rows_per_page, $total_rows = NULL, $page_number = 1, $customUri = false, $pagination_config = [])
    {
        ...
        if(!$customUri){
            ...
            $uri_array = explode('/', trim($customUri, '/'));
            $segments = count($uri_array);

            // Reset arrray key begin from 1
            array_unshift($uri_array, "");
            unset($uri_array[0]);
        }
        
        // Get Page Number
        $page_number = is_numeric($this->uri->segment($segments)) 
                       ? $this->uri->segment($segments)
                       : $page_number;

        // Set pagination configurations
        $config['base_url']           = site_url(implode('/', $uri_array));
        $config['total_rows']         = $total_rows;
        $config['per_page']           = $rows_per_page;
        $config['uri_segment']        = $segments + 1;
        ...
        $this->all_pages = $this->pagination->create_links();
    ...
    //  * @return array
    //  */
    // public function setFilter($exclude = [], $include = [])
    // {
    //     if($filter = $this->input->get('filter')){
    //         foreach ($filter as $field => $value) {
    //             if(!in_array($field, $exclude) && !empty($value))
    //                 $this->like(str_replace(':','.',$field), $value);
    //         }
    //     }
    //     if(!empty($include))
    ...
    public function setFilter()
    {
        if($filter = $this->input->get('filter', true)){
            ...
            foreach ($filter as $field => $value) {
                if(isset($this->fields[$field]['filter']))
                    $field = $this->fields[$field]['filter'];
