...
class MY_Migration
{
	...
	public function display_current_migrations()
	{
		...
		foreach ($modules as $module) {
			$this->init_module($module['slug']);
			$migrations[$module['slug']] = $this->_get_version($module['slug']);
		}
	...
	public function display_all_migrations()
	{
		...
		$migrations = array();
		foreach ($modules as $module) {
			$this->init_module($module['slug']); // call module folder name in index [1]
			$migrations[$module['slug']] = $this->find_migrations();
		}
	...
	// $mode : current|latest
	public function migrate_all_modules($mode = 'current', $enabled_module_only = false)
	{
		$modules = $this->list_all_modules_with_migrations();

		foreach ($modules as $module)
		{
			// Migrate only enable module if $enabled_module_only is true
			if($module['slug'] != 'CI_core' 
				&& $enabled_module_only 
				&& ($module['enable'] ?? null) == false)
				continue;
	...
	public function migrate_entry($entry)
	{
		ci()->load->model('entry/Entry_model');

		if(!$yaml = ci()->Entry_model->get_entry_config($entry, false, false)) {
            return ['status' => 'failed', 'message' => '<div class="alert alert-warning">Entry not found.</div>'];
        }

        if(ci()->Entry_model->sync($entry, $yaml)){
            return ['status' => 'success', 'message' => '<div class="alert alert-success">Entry structure synced successfully.</div>'];
        } else {
            return ['status' => 'failed', 'message' => '<div class="alert alert-danger">Entry structure fail to synced.</div>'];
        }
	}
	...
	public function list_all_modules_with_migrations()
	{
		...
		return array_merge(array(array('', 'CI_core', 'slug' => 'CI_core')), $modules);
	}
