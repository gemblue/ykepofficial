...
class Migration extends Backend_Controller {
	...
	private $moduleDisableFile = SITEPATH.'configs/disabled_modules.json';
	...
	function __construct()
	{
		...
		if(!file_exists(SITEPATH.'configs/')){
			...
			file_put_contents($this->moduleDisableFile, '[]');
		}
	...
	public function index($module = 'CI_core')
	{
		...
		if($module != 'CI_core')
		{
			$data['seed']['file'] = ucfirst($module).'Seeder.php';
	...
	public function generateSeeder($module = 'CI_core')
	{
		...
		if($module == 'CI_core'){
			...
			$seed_path = realpath($modules[$module][0].$modules[$module][1]).'/seeds/';
		}
	...
	{
		if(config_item('modules')[$module] ?? null){
			$classname = ucfirst($module).'Seeder';
	...
	public function enable($module)
	{
		...
		$disabledModules = json_decode(file_get_contents($this->moduleDisableFile), true);
		if (($key = array_search($module, $disabledModules)) !== false) {
    		unset($disabledModules[$key]);
		}
		file_put_contents($this->moduleDisableFile, json_encode($disabledModules));
		redirect(getenv('HTTP_REFERER'));
	}
