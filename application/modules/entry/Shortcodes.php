 ...
 */
class EntryShortcode extends LexCallback {
    ...
    public function form_elements()
    {
        ...
        $form_open  = form_open(site_url('entry/insert'), ['id' => 'form' . $entry, 'method' => 'post', 'enctype' => 'multipart/form-data']);
        ...
        if($this->config->config['entries'][$entry]['has_one'] ?? null)
        ...
        foreach($fields as $field => &$fieldOptions)
        {
            if(!isset($fieldOptions['dataform']) || isset($fieldOptions['dataform']) && $fieldOptions['dataform'] != false) {
                ...
                $element .= generate_input($fieldOptions);
                $element .= '</div>';
    ...
    public function loop()
    {
        ...
        $entryConf = $this->config->config['entries'][$entry] ?? null;
