...
<div>
  <button type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#addMultipleItemModal-<?= $config['field']; ?>">+ add item</button>
  ...
  <table class="table table-striped table-sm table-responsive" id="table-<?= $config['field']; ?>">
    <tr>
...
<!-- Modal -->
<div class="modal fade" id="addMultipleItemModal-<?= $config['field']; ?>" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
        ...
        <div class="modal-body" id="modalForm-<?= $config['field']; ?>">
          <?php foreach($savetoModel->fields as $itemField): ?>
            <?php if($itemField['field'] != $config['relation']['saveto']['foreign_key']): ?>
              <div class="form-group">
                ...
                <?= ($itemField['description'] ?? null) ? '<small> &bull; '.$itemField['description'].'</small>' : ''; ?>
        ...
        <div class="modal-footer">
          ...
          <button id="btnSave-<?= $config['field']; ?>" type="button" class="btn btn-primary">Save changes</button>
        </div>
    </div>
  </div>
</div>
<script>
  $(function(){
    $('#btnSave-<?= $config['field']; ?>').on('click', function(){
      let data = [];
      $('#modalForm-<?= $config['field']; ?>').find('input,select,radio,checkbox').each(function(i){
        data[$(this).attr('id')] = $(this).val();
      })
      let output = Object.assign({}, data);
      let outputJSON = JSON.stringify(output);

      let table = $('#table-<?= $config['field']; ?>');
      let rowTable = '<tr>';
      ...
      $('#addMultipleItemModal-<?= $config['field']; ?>').modal('close');
    });
